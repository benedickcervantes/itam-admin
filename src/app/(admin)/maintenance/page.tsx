...
import { MAINTENANCE_TOUR_STORAGE_KEY, getMaintenanceTourSteps } from "@/lib/tours/maintenance";
...
export default function MaintenancePage() {
  ...
  const { showHint, showPulse, dismissHint } = useTourHint(MAINTENANCE_TOUR_STORAGE_KEY, tourOpen, user.id);
  const startTour = () => {
    dismissHint();
    setTourOpen(true);
  };
  ...
  useEffect(() => {
    ...
    if (!shouldAutoStartTour(MAINTENANCE_TOUR_STORAGE_KEY)) return;
  ...
  const tourSteps = useMemo(() => getMaintenanceTourSteps(write), [write]);

  const handleTourStepChange = useCallback(
    (step: TourStep | null) => {
      const needsForm = Boolean(step?.id?.startsWith("service-form"));
      if (needsForm) {
  ...
  const renderRowActions = (row: MaintenanceRecord, tourTarget = false) => (
    <div className="flex items-center gap-1" {...(tourTarget ? { "data-tour": "service-actions" } : {})}>
      <button
        type="button"
        onClick={(e) => {
          e.stopPropagation();
    ...
    <>
      ...
      <div className="page-content flex-1 overflow-y-auto">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
          <div data-tour="service-search" className="w-full sm:flex-1">
            <FilterSearch
              value={searchInput}
          ...
          <div data-tour="service-filters" className="w-full sm:w-auto">
            <FilterSelect
              label="Status"
              value={status}
              onChange={(v) => {
          ...
          <div className="relative w-full sm:w-auto" ref={exportMenuRef} data-tour="service-export">
            <button
              type="button"
        ...
        {viewMode === "table" ? (
          <div className="card overflow-hidden" data-tour="service-list">
                    ...
                    <tr>
                      <td colSpan={8} className="text-slate-400">
                        <div className="flex flex-col items-start gap-1 py-1">
                          <span>No service/repair records found.</span>
          ...
          <div data-tour="service-list">
            {loading ? (
              <CardGridSkeleton />
            ) : items.length === 0 ? (
              <div className="flex flex-col items-center py-8">
                <p className="text-center text-sm text-slate-400">No service/repair records found.</p>
                <TourEmptyCta onStart={startTour} />
              </div>
            ) : (
        ...
        <Pagination page={page} totalPages={totalPages} onPageChange={setPage} total={total} itemLabel="record" />
      </div>

      <Drawer
        open={drawerOpen}
        dataTour={drawerMode === "create" && !editing ? "service-form-drawer" : undefined}
        title={
          !editing
            ? "New Service Log"
            : drawerMode === "edit"
        ...
        footer={
          drawerMode === "view" && editing && write ? (
            ...
            <div className="ml-auto flex items-center justify-end gap-2">
              ...
              <button
                type="button"
                data-tour={!editing ? "service-form-save" : undefined}
                disabled={saving}
                onClick={() => void save()}
      ...
      <SpotlightTour
        open={tourOpen}
        steps={tourSteps}
        storageKey={MAINTENANCE_TOUR_STORAGE_KEY}
