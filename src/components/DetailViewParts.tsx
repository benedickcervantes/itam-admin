...
export function DetailRow({ label, value }: { label: string; value?: string | null }) {
  ...
  if (!text) return null;
  return (
    <div className="grid gap-0.5 border-b border-slate-700/35 py-2 last:border-0 sm:grid-cols-[8.5rem_1fr] sm:gap-3 sm:py-2.5">
      <dt className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">{label}</dt>
      <dd className="text-sm leading-relaxed text-slate-100 whitespace-pre-wrap break-words">{text}</dd>
    </div>
  );
}
...
export function DetailSection({
  ...
  return (
    <section className="overflow-hidden rounded-xl border border-slate-700/70 bg-slate-900/30">
